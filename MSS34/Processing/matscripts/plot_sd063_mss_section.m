function plot_sd063_mss_section(stns,msss,variable,varargin)

% PLOT_SD063_MSS_SECTION (stns,mssstructure,variable,[property_name,property_value])
% by Povl Abrahamsen (August 2024)
% Edited for SDA063 MSS data by Ellie Fisher (7 August 2026)
% plots a basic MSS section from a cruise (with casts in a Matlab file
% named "[cruise]_mss_*_struct.mat", variable name "mss" containing an array of
...
%   - botdepth: use MSS bottom depths rather than gridded bathymetry
...
if ispc
    addpath 'L:\\work\scientific_work_areas\oceanography\MSS34\DATA\'
    addpath 'L:\\work\scientific_work_areas\oceanography\CTD\plot_transects\'
else
    addpath '/Volumes/legwork/scientific_work_areas/oceanography/MSS34/DATA/'
    addpath '/Volumes/legwork/scientific_work_areas/oceanography/CTD/plot_transects/'
end

cruise='SD063';
load([lower(cruise),'_mss.mat']);

allstations=[msss.station];
ind=zeros(size(stns));
for n=1:length(stns)
    try
...
msss=msss(ind); % Only the stations which are specified in stns

if ~isfield(msss(1).data,variable) && ~ismember(variable,{'press','corrsal','temp','ct','asal','epsilon','bvf','sig_t'})
    error('Cannot find variable %s in cruise %s',variable,cruise);
end

%now calculate all variables
maxP = 6250;
dp=nanmedian(diff(msss(1).data.press)); % Pressure difference between bins? 
start_off=dp-1; % start offset for pressure bins

tmp=nan(ceil(maxP/dp),length(stns));
...
for n=1:length(stns)
    dest_ind=[1:length(msss(n).data.press)]+round((msss(n).data.press(1)-start_off)/dp);
    press(dest_ind,n)=msss(n).data.press;
    corrsal(dest_ind,n)=msss(n).data.corrsal;
    temp(dest_ind,n)=msss(n).data.temp;
    asal(dest_ind,n)=msss(n).data.asal;
    ct(dest_ind,n)=msss(n).data.ct;
    epsilon(dest_ind,n)=msss(n).data.epsilon;
    bvf(dest_ind,n)=msss(n).data.bvf;
    sig_t(dest_ind,n)=msss(n).data.sig_t;
    if ~ismember(variable,{'press','corrsal','temp','ct','asal','epsilon','bvf','sig_t'})
        newvar(dest_ind,n)=msss(n).data.(variable);
    end
end
...
press=repmat([press(1:lastind);newpress'],1,length(msss));
corrsal=[corrsal(1:lastind,:);nan(length(press)-lastind,length(msss))];
temp=[temp(1:lastind,:);nan(length(press)-lastind,length(msss))];
asal=[asal(1:lastind,:);nan(length(press)-lastind,length(msss))];
ct=[ct(1:lastind,:);nan(length(press)-lastind,length(msss))];
epsilon=[epsilon(1:lastind,:);nan(length(press)-lastind,length(msss))];
bvf=[bvf(1:lastind,:);nan(length(press)-lastind,length(msss))];
sig_t=[sig_t(1:lastind,:);nan(length(press)-lastind,length(msss))];
if ~ismember(variable,{'press','corrsal','temp','ct','asal','epsilon','bvf','sig_t'})
    newvar=[newvar(1:lastind,:);nan(length(press)-lastind,length(msss))];
end

%data = msss(:).data;
%lat=extractfield(data,'lat');
%lon=extractfield(data,'lon');
lat=[msss.lat];
lon=[msss.lon];
...
m_proj('mercator','lon',[-34 -28],'lat',[66 68.7]);
...
switch (lower(xvar))
    ...
    case 'lat'
        ...
        plot_end_dist=plot_x(end);
    case 'date'
        plot_x=[datetime(msss.time,'ConvertFrom','datenum')];
        plot_end_dist=plot_x(end);
end


switch(variable)
    case {'asal','ct','temp','corrsal','press','epsilon','bvf'}
...
switch plottype
    case 'pcolor'
        plot_x=plot_x(:)';
        mid_x=[(plot_x(1)-plot_x(2))./2,(plot_x(1:end-1)+plot_x(2:end))./2,(3*plot_x(end)-plot_x(end-1))./2]; % 3 elements, need this explained
...
for n=1:length(stns)
    plot(plot_x(n).*[1,1],msss(n).press([1,end]),'k');
end
if station_labels
...
if make_chartlet
    ...
    buffer_m=750;
    dlon=[min(lon) max(lon)]+[-1 1].*buffer_m./1852./60./cos(mean(lat).*pi./180);
    dlat=[min(lat) max(lat)]+[-1 1].*buffer_m./1852./60;
