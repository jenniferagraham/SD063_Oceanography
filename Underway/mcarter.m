function cordep = mcarter(lat,lon,uncdep)
% function cordep = mcarter(lat,lon,uncdep)
%
...
%
% OUTPUT:
%   cordep is corrected depth in metres
%
% EXAMPLES:
...
%   Un-mstarred by EPA 2021-02-08 on RRS James Cook
...
if ~exist(fullfile(myfolder,'mcarter_data.mat'),'file')
    prevdir=pwd;
    cd(myfolder)
    make_carter;
    cd(prevdir)
...
%     if isfield(MEXEC_G,'Mcarter') ~= 1; ok = 0; break; end% database appears not to have been built
    if isfield(d,'c_area') ~= 1; ok = 0; break; end
    if numel(d.c_area) ~= 64800; ok = 0; break; end
    if d.c_area(32490) ~= 23; ok = 0; break; end % check value at lat = 0, lon = 0
    if isfield(d,'c_corrected') ~= 1; ok = 0; break; end
    if numel(d.c_corrected) ~= 85; ok = 0; break; end % 85 carter areas
    pcheck = d.c_corrected{23};
...
%     m2 = 'results stored in global structure variable d';
...
lat(lat < -90) = -90;
lat(lat >= 90) = 89.5;
lon  = mod(lon+180,360)-180; %mcrange(lon,-180,180);
klati = floor(lat) + 91;
kloni = floor(lon) + 181;
...
for k = 1:numel(uncdep)
    ...
    corrprof = d.c_corrected{carea(k)};
