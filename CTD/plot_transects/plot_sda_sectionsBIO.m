...
%% LC writting this temporal script too look at bio sensors 
  %'fluor_ug_l','BeamTrans','par','oxygen_umol_kg'
...
% specifics for each section, depth, caxis, etc
    ...
    P.maxy=100; % force the preassure to the surface waters 

    % Plot phytoplankton biomass 'fluor_ug_l',
    subplot(3,5,3:5);
    ...
    ylabel('Depth (m)');
    hcb=colorbar;
    hcb.Label.String= 'fluor ug l^{-1}'; 
    sectionlength=max(xlim);
    
    %%%% choose one here 
    % 'par',
    ax2=subplot(3,5,8:10);
    plot_sk_ctd_section(P.sectionlist,ctds,'par','xvar','dist','type','pcolor_interp','levels',[],...
        grid_options{:});
    ...
    % %'BeamTrans' or 
     ax3=subplot(3,5,13:15);
    plot_sk_ctd_section(P.sectionlist,ctds,'BeamTrans','xvar','dist','type','pcolor_interp','levels',[],...
        grid_options{:});
     cmocean('gray')
    ylim([0 P.maxy]);
    ylabel('Depth (m)');
    hcb=colorbar;
    hcb.Label.String='Beam Transmissivity';
