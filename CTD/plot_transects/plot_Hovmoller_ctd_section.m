function plot_Hovmoller_ctd_section(stns,ctds,variable,varargin)

%Created by Rosie Williams, based on code in plot_sk_ctd_section
% Comments not edited yet!
...
switch plottype
    ...
    case 'pcolor_interp'
        ...
        % Paul Holland: create mask to find out where bottom triangles of plot field 
        ...
        % Paul Holland: plot contour showing areas where bottom triangles
        % are questionably interpolated
        contour(plot_dist-median(diff(plot_dist))./2,press(:,1),bot_tri_mask,[0.5 0.5],'m','LineWidth',1);

        % Paul Holland: in bottom triangles, extrapolate horizontally using
        % nearest neighbour
        % plot_var_interp(bot_tri_mask==1)=NaN;
        % for k=1:size(plot_var_interp,1)
        %    plot_var_depthslice=plot_var_interp(k,:);
        %    valid=find(isfinite(plot_var_depthslice));
        %    plot_var_interp(k,:)=interp1(plot_dist(valid),plot_var_depthslice(valid),plot_dist, 'nearest')            
        % end
        
        % Paul Holland: in bottom triangles, preserve gradient of last
        % common depth and fill downwards on that basis
        % TO BE CODED
