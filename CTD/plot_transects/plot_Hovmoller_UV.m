...
for ii=1:ncasts
    ...
    u=ctds(P.sectionlist(ii)).ladcp_u;
    v=ctds(P.sectionlist(ii)).ladcp_v;
    uS(:,ii)=sqrt(u.^2 + v.^2).*sign(u);
...
u_array=NaN(size(uS,1),length(t30));
...
uS_padded=NaN(size(uS,1),2*ncasts);

for ii=1:ncasts 
 x_interval(1,2*ii-1)=x_start(ii);
 x_interval(1,2*ii)=x_end(ii);
 uS_padded(:,2*ii-1)=uS(:,ii);
end

%ax2=subplot(3,1,1)
axes(ha(1))
%pcolor(x, pressS(:,1), CTempS)
pcolor(x_interval, pressS(:,1), uS_padded);
...
u_mean = mean(uS, 2, 'omitnan');
u_std = std(uS,0,2,'omitnan');
...
% Temperature anomaly
u_anom = uS - u_mean;
u_padded_anom = uS_padded - u_mean;
...
yyaxis right
plot(x,chosen_u,'r*');
ylabel(sprintf('uSpeed anomoly at %d m',chosen_press));
ylim([-0.1  0.1])
...
pcolor(x_interval, pressS(:,1), u_padded_anom)
shading flat
set(gca,'YDir','reverse')
...
for n=1:3
 ...
 if yoyo_zoom
name = sprintf('_Hovmoller_uSpeedanomolies_%s_full.png', sectionfilename);
 else
     name = sprintf('_Hovmoller_uSpeedanomolies_%s.png', sectionfilename);
 end
