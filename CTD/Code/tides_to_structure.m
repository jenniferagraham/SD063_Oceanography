...
if mac==0 % ellie you should be able to run it using mac=0 you may need to adjust some of the paths
    ...
    strucsavepath='';
    logsavepath='L:\work\scientific_work_areas\oceanography\MSS34\';
elseif mac==1
    slash='/';
    ...
    mssdataP = [disk,'oceanography/MSS34/DATA/'];
    addpath '/Volumes/legwork/scientific_work_areas/oceanography/Gr1kmTM/data/';
    addpath 'L:/work/scientific_work_areas/oceanography/TMD3.0'
    strucsavepath='';
    logsavepath='/Volumes/legwork/scientific_work_areas/oceanography/MSS34/';
end
...
load (fullfile(mssdataP,"sd063_mss.mat")) % use lower(cruise) instead - change formatting
...
for s=1:length(stns)
    ...
    lat = msss(s).data.lat; % Use point-specified latitude
    lon = msss(s).data.lon; % Use point-specified longitude
    % Predict the entire tide time series for this location:
    z = tmd_predict(fullfile(disk,'oceanography\Gr1kmTM\data\Gr1kmTM_v1.nc'),68.2796,-30.7665,times);
...
%% Export to new CSV
%save([logsavepath,savename],'mssLogFixed')